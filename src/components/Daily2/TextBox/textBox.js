...
const TextBox = ({dailyData}) => {
  ...
  const [selectedAttributeNames, setSelectedAttributeNames] = useState(dailyData.selectedAttributeNames);
// console.log("one graph? ", daily)
...
const dailyWithTitle = () =>{
  ...
  const RightSpan = ({ text, fieldName, color }) => {
    ...
    return (
      <span style={{ background: `${defaultColor}` }} className={classes.rightSpan}>
        {text || ''}
      </span>
    );
  };
  ...
  return daily && daily.selectedDateData ? (
    <div className={classes.textItem}>
      <div className={classes.infoWrap}>
        <h3>{new Date(daily.selectedDateData.Date).toString().substring(0, 16)}</h3>
      </div>
      {renderRightSpans()}
